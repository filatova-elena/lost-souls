#!/usr/bin/env python3
"""
Character Card Print Sheet Generator
...
# Import generate_card functions
sys.path.insert(0, str(Path(__file__).parent))
from generate_card import build_html, render_card, find_image, BASE_URL, load_skills_data
...
def generate_card_image(yaml_path, scale=3, base_url=BASE_URL, skills_data=None):
    """Generate a single character card as a PIL Image."""
...
def make_print_sheet(
    ...
    """
    Generate a high-res PNG print sheet of character cards.
    ...
    per_page = cols * rows
    num_pages = math.ceil(len(character_yamls) / per_page)

    # Load skills data
    project_root = Path(character_yamls[0]).resolve()
    for _ in range(10):
    ...
    print(f"Cards: {len(character_yamls)}  |  Pages: {num_pages}")
    print()
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate high-res PNG print sheets of character cards",
